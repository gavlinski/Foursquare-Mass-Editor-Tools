...
### Overview
O Playwright MCP usa Chromium headless para navegar na aplicação. Para acessar páginas autenticadas, o agente usa tokens capturados do browser do host (Brave) via CDP.

### Arquivos do Sistema MCP
| Arquivo | Propósito |
|---------|----------|
| `.vscode/mcp.json` | Configuração do servidor Playwright MCP |
| `scripts/mcp-playwright-prepare.sh` | Instala Chromium persistente no workspace |
| `scripts/mcp-foursquare-auth-bootstrap.sh` | Captura token via CDP do Brave |
| `scripts/mcp-foursquare-auth-clear.sh` | Remove storage state (forçar re-auth) |
| `scripts/get-foursquare-token.js` | Extrai token para uso do agente |
| `data/mcp/playwright/foursquare.storage-state.json` | Cookies salvos (git-ignored, chmod 600) |
| `data/mcp/playwright/browsers/` | Chromium instalado (git-ignored, persistente) |

### Padrão de Autenticação para o Agente

**IMPORTANTE:** O `--storage-state` do Playwright MCP não carrega cookies automaticamente. O agente deve **injetar o token manualmente** após cada `browser_navigate`.

```javascript
// 1. Navegar para a página do domínio
await page.goto('https://localhost/4sqmet/index.php');

// 2. Obter token do storage state via terminal
// node /var/www/html/scripts/get-foursquare-token.js

// 3. Injetar o token via evaluate
await page.evaluate(() => {
  document.cookie = 'oauth_token=<TOKEN>; path=/; secure; samesite=lax';
});

// 4. Navegar para a página autenticada
await page.goto('https://localhost/4sqmet/main.php');
// → Status: Conectado ✅
```

### Bootstrap Initial (Uma Vez / Quando Token Expirar)

```bash
# 1. macOS host: abrir Brave com remote debugging
/Applications/Brave\ Browser.app/Contents/MacOS/Brave\ Browser \
  --remote-debugging-port=9222 \
  --user-data-dir=/tmp/brave-mcp-profile

# 2. Dev Container: executar bootstrap
./scripts/mcp-foursquare-auth-bootstrap.sh
# → Fazer login no Foursquare no Brave
...
> Para detalhes completos, troubleshooting e padrões de uso: load skill **mcp-playwright-auth**.
