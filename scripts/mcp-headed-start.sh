#!/bin/bash
# mcp-headed-start.sh
# Wrapper para playwright-headed: busca a WS URL do Brave e transforma localhost → gateway IP:9222
...
# O MCP tentaria conectar na porta 80 (Apache). Precisamos substituir pelo gateway IP:9222.
...
if [ -z "$CDP_HOST_IP" ]; then
    echo "ERROR: Não foi possível resolver host.docker.internal" >&2
    exit 1
fi
...
# Extrai a WS URL e substitui localhost → gateway IP:9222
WS_URL=$(echo "$JSON" \
    | grep -o '"webSocketDebuggerUrl": *"[^"]*"' \
    | grep -o 'ws://[^"]*' \
    | sed "s|ws://localhost/|ws://${CDP_HOST_IP}:9222/|g")
